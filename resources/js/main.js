...
$('#buscarGramatical').change(function(){
    clearTimeout(timer);
    timer = setTimeout(buscarGramatical, 500)
});
...
/*  Gramatical
*/
    var listarGramatical = function() {
        $.ajax({
            type: "get",
            url: "/gramatical/listar",
            success: function(data) {            
                $("#ListaGramatical").empty().html(data);            
            }
        });
    };  

    var buscarGramatical = function() {
        var x = document.getElementById("buscarGramatical");
        x.value = x.value.toUpperCase();
        if (x.value == '') {
            listarGramatical();
        } else {
    ...
    var nuevaGramatical = function() {
        $.ajax({
            type: "get",
            url: "/gramatical/nuevo",
            success: function(data) {  
                $('#tituloModalMd').empty().html('<i class="fas fa-font"></i> Nueva gramatical');
                $('#contentModalMd').empty().html(data);       
                $('.actionBtn').addClass('grabarGramatical'); 
                $("#myModalMd").modal({backdrop: "static"});
            }
        });
    };

    var editarGramatical = function(gramatical_id) {
        $.ajax({
            type: 'get',
            url: '/gramatical/'+gramatical_id+'/editar',
            success: function(data) {  
                $('#tituloModalMd').empty().html("<i class='fa fa-rocket'></i> Editar gramatical");
                $('#contentModalMd').empty().html(data);         
                $('.actionBtn').addClass('actualizarGramatical'); 
                $('#myModalMd').modal({backdrop: 'static'});
            }
        });
    };

    var eliminarGramatical = function(id) 
    {    
        if (confirm('Seguro de Eliminar esta gramatical?')) {
            $.ajax({
                url: '/gramatical/'+id+'/eliminar',
                type: "get",
                success: function (data) {
                    listarGramatical();
                    Toast.fire({
    ...
    $(document).on("click", "#pg-gramaticales .pagination li a", function(e) {
        e.preventDefault();
        var url = $(this).attr("href");
        $.ajax({
            type: 'get',
    ...
    $('.modal-body').on('click', '.grabarGramatical', function(event){
        $('.grabarGramatical').prop('disabled', false);
        event.preventDefault();
        var data = $( "#formNuevaGramatical" ).serialize();
        $.post( "/gramatical/grabar", data, function(data){
            if ($.isEmptyObject(data.error)) {  
                $(".panel-collapse").collapse('hide');
                listarGramatical(); 
                $("#myModalMd").modal("hide");  
    ...
    $('.modal-body').on('click', '.actualizarGramatical', function(event){
        $('.actualizarGramatical').prop('disabled', true); 
        event.preventDefault();
        var data = $( "#formActualizarGramatical" ).serialize();
        var id = $( "#gramatical_id" ).val();        
        $.post( "/gramatical/"+id+"/actualizar", data, function(data){            
            if ($.isEmptyObject(data.error)) {
                listarGramatical();
                $("#myModalMd").modal("hide");                
                Toast.fire({
                ...
                $('.actualizarGramatical').prop('disabled', false);
            }
        });   
    });
